...
import newPoser from '../models/poser.model'
 ...
 */
 const createPoser = async (req: Request, res: Response, next: NextFunction) => {
    const baseUser: BaseUser = createUser();

    const contact = createContact(baseUser.userName);
    const addressStr: string = ''.concat(
        ...Object.values(createAddress()).map((addrPart) => {
            return `${addrPart}, `;
        })
    );
    ...
    console.log(`Attempting to save: ${poserInput}`); 

    await PoserModel.create(poserInput, function(err, savedPoserInput) {
        if(err) {
            logger.info(`Couldnt save poser: ${err.message}`)
            next(err); 
        } else {
