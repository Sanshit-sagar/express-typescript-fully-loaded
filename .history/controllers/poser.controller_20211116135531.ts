...
const updateRole = async (req: Request, res: Response) => {
    ...
    if(!firstName || !lastName || !userName || !email) {
        res.status(422).json({ message: `The fields firstName, lastName, userName and description are required` });
    }

    await PoserModel.updateOne({ _id: id }, { firstName, lastName, userName, email, avatar }); 

    const roleUpdated = await PoserModel.findById(id, { firstName, lastName, userName, email, avatar }); 
