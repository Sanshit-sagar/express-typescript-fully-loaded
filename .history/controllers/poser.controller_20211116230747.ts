 ...
 */
 const createPoser = async (_req: Request, res: Response, next: NextFunction) => {
    ...
    logger.info(`Attempting to save: ${poser}`); 
    ...
    await PoserModel.create(poser, function(err, _poserInput) {
        ...
        if(err) {
            logger.info(`Encountered an error: ${err.message}`next(err); 
        else next(); 
    });
}
 ...
 * GET /api/posers
 */
const getAllPosers = async (_req: Request, res: Response, next: NextFunction) => {
