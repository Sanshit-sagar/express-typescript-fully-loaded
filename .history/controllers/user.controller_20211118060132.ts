...
type Comparator = '+createdAt' | '-createdAt' | '-email' | '+email';
type SortByField =  'email' | 'createdAt';

const sanitizeComparator = (dir: SortDirection | string, comp: SortByField | string): Comparator => {
    const c: SortByField = comp==='email' ? 'email' : 'createdAt';
    const d: Extract<SortDirection, "+" | "-"> = dir.length > 1 
        ? dir==='desc'  ? '-' : '+'
        : dir==='+'     ? '+' : '-';
    
    return `${d}${c}`;
}
const sanitizeLimit = (limit: any) => parseInt(String(limit));
...
const getAllUsers = catchAsync(async (req: Request, res: Response) => {
    ...
    const count: number | typeof NaN = sanitizeLimit(limit);
    ...
    if(errorStr.length)  throw new ExtendedError(400, errorStr); 
