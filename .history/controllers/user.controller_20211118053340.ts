...
const getAllUsers = catchAsync(async (req: Request, res: Response) => {
    ...
    if(!isValidLimit(parseInt(String(limit)))) {
    ...
    if(isValidSortComparator(String(sortBy))) {
        throw new ExtendedError(400, 'Invalid sortBy, must be email or createdAt');
    }


    const users = await User.find().sort(`${sortDirection==='desc' ? '-' : '+'}${sortBy}`).limit(parseInt(String(limit))).exec(); 
