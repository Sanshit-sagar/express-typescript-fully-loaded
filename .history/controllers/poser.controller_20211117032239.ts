...
import { sanitize } from 'lib/faker/modelFactory'
...
type GetAllPosersQuery = {
    limit: number;
    sort: `${'+' | '-'}${'email' | 'firstName' | 'lastName' | 'userName' | 'phoneNumber'}`;
}
...
const getAllPosers = async (req: Request, res: Response, next: NextFunction) => {
    ...
    sanitizeGetAllPosersQuery(req.query)
