...
const sanitizeComparator = (dir: SortDirection | string, comparator: SortByField | string): Comparator => {
    
    let dir: "+" | "-" = (direction.length > 1) 
        ? (direction==='desc' ? '-' : '+') 
        : (direction==='+' ? '+' : '-');

    let comp: "email" | "createdAt" = comparator==='email' 
        ? 'email' 
        : 'createdAt';
