...
const getMissingFieldsArr = (fields: { [key: string]: string; }): string[] => {
    let res = requiredFields.map((rf) => {
        if(!fields[rf]) {
            return rf; 
        } 
    });
...
const getMissingFields = (fields) => getMissingFields(fields).join(', ').trim()
...
const createUser = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email || !password || !firstName || !lastName) {
        throw new ExtendedError(
            400, 
            `Please provide the following fields: ${ge}`, 
