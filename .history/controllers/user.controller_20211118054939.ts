...
const sanitizeComparator = (
    ...
    `${direction==='desc' ? '-' : '+'}${comparator==='email' ? 'email' : 'createdAt'}`
);
...
const getAllUsers = catchAsync(async (req: Request, res: Response) => {
    ...
    const comparator = sanitizeComparator(String(sortBy), String(sortDirection))
    ...
    if(errorStr.length) {
        console.log(`About to throw: ${errorStr}-${count}-${comparator}`)
        throw new ExtendedError(400, errorStr); 
    }
    console.log(`Sanitized values${count}-${comparator}`)
