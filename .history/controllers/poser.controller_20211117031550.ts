...
const getAllPosers = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allPosers = await PoserModel.find();
    const queriedPosers = limit?.length ? allPosers.limit(limit) : allPosers;
    const sortedPosers = .sort(sort).exec()
        : allPosers.sort(sort).exec(); 
