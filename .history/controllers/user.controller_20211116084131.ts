...
const createUser =  async (req: Request, res: Response) => {
    ...
    const userCreated = await User.create(userInput, function(err, userInput) {
        if(err) {
            res.status(409).json({
                message: `The params used are in conflict with an existing resource`,
                locationHeader: `${req.pa`
            })
        }
    })
...
const getAllUsers = async (req: Request, res: Response) => {
    ...
    const users = await User.find().limit(parseInt(String(limit))).sort({ 'email': `${sort}` }).exec(); 
