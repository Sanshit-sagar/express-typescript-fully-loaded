 ...
 */
 const createPoser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userName: Pick<BaseUser, 'userName'> = baseUser;

    const contact = createContact(userName);
    const address = createAddress();
    const addressStrArr = Object.values(address).map((addrPart) => `${addrPart}`) 
    const addressStr: string = ''.concat(...addressStrArr);
