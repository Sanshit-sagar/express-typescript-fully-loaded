 ...
 */
 const createPoser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const poserInput: PoserInput = { firstName, lastName, userName, email, avatar }; 

    logger.info(`Attempting to create: ${JSON.stringify(poserInput)}`);

    const createdPoser = await PoserModel.create(poserInput, function(err, savedPoserInput) {
        if(err) {
