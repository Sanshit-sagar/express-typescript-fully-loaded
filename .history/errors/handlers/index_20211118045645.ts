...
const handleUnprocessableEntityError = (err, res) => {
    return res.send(409).json({ message: err.message || "yeyeye" })
}
...
export default ((err: any, req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(err.name === "Unprocessable Entity") return err = handleUnprocessableEntityError(err, res)
        ...
        elsreturn err = handleUnknownServerError(err, req, res); 
    } catch(err) {
