...
const httpCauses = Object.values(httpStatusCodes).map((details) => Object.entries(details)[0][1]); 

type HttpCodeMap = typeof httpStatusCodes; 
type HttpStatusCode = keyof HttpCodeMap;
type HttpStatusMessage = typeof HttpCodeMap[keyof HttpCodeMap].| string;
...
export default ((error, req: Request, res: Response, next: NextFunction) => {
    ...
    errorMessage = errorMessage || httpStatusCodes[errorResponse.statusCode].cause; 
