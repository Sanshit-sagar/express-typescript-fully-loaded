...
const locale: any | validator.PostalCodeLocale = "US"; 
...
export default {
  ...
  email:    (rule: boolean, value: string = "")     =>  rule === true ? validator.isEmail(value):!validator.isEmail(value);
  },
  equals:   (rule: string, value: string = "")      =>      validator.equals(value, rule),
  matches:  (rule: string, value: string = "")      =>      rule === value,
  minLength: (rule: number, value: string = "")     =>      value.length >= rule,
  ...
  vat:      (rule: boolean, value: string = "", countryCode: string = "US") => {
    return rule === true 
        ?    validator.isVAT(value, countryCode) 
        :   !validator.isVAT(value, countryCode);
  },
};