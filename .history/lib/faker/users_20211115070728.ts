...
export const createHackerDetails = (addressBatchSize: number): HackerDetails[] => {
    if(addressBatchSize <= 1) throw new Error('createHackerDetails() requires > 1 addresses to be created.');
...
type UserPropSubsets = Address[] | HackerDetails[]; 
type UserExtensions = Partial<Address> & Partial<HackerDetails>

const collateUsers = (baseUsers: BaseUser[], indexedProperties: UserPropSubsets[]): UserExtensions[] => {
