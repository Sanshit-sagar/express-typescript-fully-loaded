...
const collateUsers = (baseUsers: BaseUser[], indexedProperties: UserDetailSubsets[]) => {
    return baseUsers.map((baseUser, userIndex) => {
        return {
            ...baseUser,
            extractPropsForUserAtIndex(indexedProperties, userIndex)
        };
    })
}

const extractPropsForUserAtIndex = (subsets: UserDetailSubsets[], index: number): UserDetails => {
    let userDetails: UserDetails = subsets.map((propArray: UserDetailSubset, j: number) => {
        let detailForAllUsers = propArray[j];
        return ...prop
    })
}
