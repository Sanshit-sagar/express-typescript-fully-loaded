...
function handleLog(connectionType: 'open' | 'close', socketServer) {
    return (typeof connectionType === 'close')`Socket closed. Number of clients: ${socketServer.clients.size} (-1)`
}
...
export default async (app: Express) => {
    ...
    socketServer.on("connection", function connection(socketClient: WebSocket, connectionRequest: IncomingMessage) {
        ...
        socketClient.on("close", (_socketClient) => {
            console.log(handleLog('close', socketServer)); 
        })
    }); 
