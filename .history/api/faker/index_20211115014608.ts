...
const isUsernameSluggable = (username: string) => {
    return !sluggableUsername.includes('.') && !sluggableUsername.includes('_')
}

const getSluggableUsername = (): string => {
    let sluggableUsername = faker.internet.userName();
    while(!isUsernameSluggable(sluggableUsername)) {
        sluggableUsername = faker.internet.userName();
    }
    return sluggableUsername;
}
