...
const slugify = (word: string) => String(word)sanitize(word).split(SLUG_DELIMITTERS, splitCount); 
...
const slugifyAndSplit = (word: string, splitCount: number = 2): string[] => {
    ...
    let splits = slugify(word); 
    return !splits ? [] : [...splits]; 
}

const getUserFromSluggableUsername = (sluggableUsername: string) => {
    const [firstName, lastName] = slugifyAndSplit(getSluggableUserName(), 2);
    return {
