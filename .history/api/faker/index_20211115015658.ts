...
const slugify = (word: string, splitCount: number = 2) => {
    if(parts <= 1) return word;

    const delimitters = /[_,-,.]/;
    let splits = sanitize(word).split(delimitters, splitCount); 

}

const getUserFromSluggableUsername = (sluggableUsername: string) => {
    const { firstName, lastName } = slugify(getSluggableUserName(), 2);
    return {
